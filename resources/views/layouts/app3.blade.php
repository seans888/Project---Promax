...
<div class="wrapper">
  ...
  <header class="main-header">
    ...
    <a href="/" class="logo">
      ...
      <span class="logo-mini"><b>{{Auth::user()->company->name}}</b></span>
      <!-- logo for regular state and mobile devices -->
      <span class="logo-lg"><b>{{Auth::user()->company->name}}</b></span>
    </a>
        ...
        <ul class="nav navbar-nav">
          ...
          <li class="dropdown user user-menu">
            <a href="#" class="dropdown-toggle" data-toggle="dropdown">
              ...
              <span class="hidden-xs">{{ (Auth::user()->employee) ? Auth::user()->employee->name : Auth::user()->name }}</span>
            </a>
            <ul class="dropdown-menu">
              ...
              <li class="user-header" style="height:100px">
                ...
                <p>
                  {{(Auth::user()->employee) ? Auth::user()->employee->name: Auth::user()->name}} - {{Auth::user()->usertype->desc}}
                  <small>Registered since {{Auth::user()->created_at->format('M j, Y')}}</small>
                </p>
              </li>
  ...
  <aside class="main-sidebar">
    ...
    ?>
      ...
      @if($collectionAccessLevels->where('process', 'enter')->count() > 0)
        <?php
        $areActiveRoutes = [];
        ...
        <li class="{{ areActiveRoutes($areActiveRoutes) }} treeview">
          ...
          <ul class="treeview-menu">
            @foreach($collectionAccessLevels->where('process', 'enter') as $AccessLevel)
              <li class = "{{isActiveRoute($AccessLevel->code)}}">
                <a href="{{$AccessLevel->uri}}">
                  <i class="fa {{$AccessLevel->icon}}"></i> 
                  {{$AccessLevel->uifieldname}}</a></li>
           @endforeach
            
          </ul>
        </li>
      @endif
      @if($collectionAccessLevels->where('process', 'process')->count() > 0)
        <?php $areActiveRoutes = [];
      ...
      @endif
      @if($collectionAccessLevels->where('process', 'reports')->count() > 0)
        <?php
        $areActiveRoutes = [];
        ...
        <li class="{{ areActiveRoutes($areActiveRoutes) }} treeview">
          ...
          <ul class="treeview-menu">
             @foreach($collectionAccessLevels->where('process', 'reports') as $AccessLevel)
              <li class = "{{isActiveRoute($AccessLevel->code)}}">
                <a href="{{$AccessLevel->uri}}" target="_blank" rel=”noopener noreferrer”>
                  <i class="fa {{$AccessLevel->icon}}"></i> 
                  {{$AccessLevel->uifieldname}}</a></li>
           @endforeach
          </ul>
      ...
      <?php $areActiveRoutes = [];
        ...
        <li class="{{ areActiveRoutes($areActiveRoutes) }} treeview">
          ...
          <ul class="treeview-menu">
            <li class = "{{isActiveRoute('myaccount')}}">
                <a href="/myaccount">
...
$("#startdateparam").change(function(){
  location.href = "?startdate=" + $("#startdateparam").val();
});
